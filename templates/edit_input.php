...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT Title, drop_file, short_description, description FROM input_file WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt->num_rows > 0) {
            $stmt->bind_result($title, $dropFile, $shortDescription, $description);
            $stmt->fetch();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE input_file SET Title = ?, drop_file = ?, short_description = ?, description = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->bind_param("ssssi", $title, $dropFile, $shortDescription, $description, $id);
